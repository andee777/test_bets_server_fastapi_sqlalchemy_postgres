...
async def auto_place_bets_favourite_at_mins_75(session: AsyncSession):
    ...
    for match in live_matches:
        ...
        subq = select(BetEvent.match_id).join(Bet, BetEvent.bet_id == Bet.bet_id).where(
            and_(
                Bet.bot == True,
                Bet.user_id == 2,
                Bet.bot_task.ilike("bet_favourite_at_mins_75"),
                BetEvent.match_id == match.match_id
            )
        )
        exists_bet = await session.execute(select(exists(subq)))
        if exists_bet.scalar():
            # logger.info(f"[75+] Bot bet already placed for match {match.match_id}, skipping.")
            continue
        ...
        if is_valid(latest_odd.home_win):
            ...
            # logger.info(f"[75+] No qualifying low odds for match {match.match_id}. Skipping.")
            continue
        ...
        bet_payload = {
            ...
            "bot": True,
            "bot_task": "bet_favourite_at_mins_75"
        }
        ...
        logger.info(f"\n ----- [75+] Placed bot bet on {bet_type.upper()} for match {match.match_id} with odd {odd_value} -----")

    await session.commit()

async def periodic_auto_bet_favourite_at_mins_75():
    while True:
        # logger.info("Running 80+ minute auto-bet task.")
        async with async_session() as session:
            try:
                await auto_place_bets_favourite_at_mins_75(session)
            except Exception as e:
                logger.error(f"Error in 75+ min bet placement: {e}")
                await session.rollback()
        # Run the task every 10 minutes (600 seconds)
        await asyncio.sleep(60)

# Uncomment to run this task directly:
# if __name__ == '__main__':
#     asyncio.run(periodic_auto_bet_late_game())
