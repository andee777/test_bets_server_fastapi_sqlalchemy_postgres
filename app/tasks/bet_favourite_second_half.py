...
async def auto_place_bets_for_live_matches(session: AsyncSession):
    """
    ...
    for match in live_matches:
        ...
        subq = select(BetEvent.match_id).join(Bet, BetEvent.bet_id == Bet.bet_id).where(
            and_(
                Bet.bot == True,
                Bet.bot_task.ilike("bet_favourite_second_half"),
        ...
        def is_valid_range(val_init, val_latest):
            return (val_init is not None and val_latest is not None and
                    1 <= val_init <= 2 and 1 <= val_latest <= 2)
        ...
        bet_payload = {
            ...
            "bot": True,
            "bot_task": "bet_favourite_second_half"
        }
...
async def periodic_auto_bet():
    while True:
        logger.info("Running automated bet placement task.")
        async with async_session() as session:
            try:
                await auto_place_bets_for_live_matches(session)
