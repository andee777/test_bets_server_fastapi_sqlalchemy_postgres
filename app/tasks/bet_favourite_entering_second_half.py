...
async def auto_place_bet_favourite_entering_second_half(session: AsyncSession):
    """
    ...
    for match in live_matches:
        ...
        subq = select(BetEvent.match_id).join(Bet, BetEvent.bet_id == Bet.bet_id).where(
            and_(
                Bet.bot == True,
                Bet.bot_task.ilike("bet_favourite_entering_second_half"),
        ...
        def is_valid_range(val_init, val_latest):
            return (val_init is not None and val_latest is not None and
                    1 <= val_init <= 100 and 1 <= val_latest <= 1.3)
        ...
        if is_valid_range(initial_odd.home_win, latest_odd.home_win):
            ...
            # logger.info(f"[45+] No qualifying odds for match {match.match_id}. Skipping.")
            continue
        ...
        bet_payload = {
            ...
            "bot": True,
            "bot_task": "bet_favourite_entering_second_half"
        }
        ...
        logger.info(f"\n ----- [45+] Placed bot bet on {selected_type.upper()} for match {match.match_id} at odd {odd_value} -----")

    await session.commit()

async def periodic_auto_bet_favourite_entering_second_half():
    while True:
        # logger.info("Running automated bet placement task.")
        async with async_session() as session:
            try:
                await auto_place_bet_favourite_entering_second_half(session)
